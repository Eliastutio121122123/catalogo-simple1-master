...
from ..odoo.reviews import review_service
...
class StoreProductReviewsAPI(StoreBase):
    """GET /store/products/<id>/reviews — list approved reviews (public).
    POST /store/products/<id>/reviews — create a review (JWT required)."""
    ...
    def post(self, product_id: int):
        ...
        # Decode JWT manually so that GET stays public
        ...
        data = request.get_json() or {}
        try:
            new_review = review_service.create_review(uid, product_id, data)
            return success(new_review, 201)
...
bp.add_url_rule(
    ...
    view_func=StoreProductReviewsAPI.as_view("store_product_reviews"),
)
